...
const displayMovements = function (acc, sort = false) {
  ...
  const movsDates = acc.movements.map((mov, i) => ({
    movement: mov,
    date: acc.movementsDates.at(i),
  }));

  if (sort) {
    movsDates.sort((a, b) => a.movement - b.movement);
  }
  ...
  movsDates.forEach(function (obj, i) {
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    //In each callback, print the remaining time to the UI
    labelTimer.textContent = `${minutes}:${sec}`;

    //Decrease time

    //After reaching 0 seconds, stop timer and logout user
    if (time === 0) {
      clearInterval(timer);
